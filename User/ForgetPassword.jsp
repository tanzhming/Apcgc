...
<div class="mui-content" style="margin-top:10px;">
    ...
    <div class="mui-content-padded">
        <button id='sendMail' class="mui-btn mui-btn-block mui-btn-primary" onclick="regsubmit()">下一步</button>
    </div>
</div>
</div>
...
<script type="text/javascript">
    ...
    function regsubmit() {
        ...
        if (account == "") { mui.alert("请输入用户名", '提示', function () { }); return false; }
        /* $.post('/User/ForgetPasswordSubmit', { UserID: account },
          function (data) {
              if (data.success) {
                  window.location.href = '/User/ForgetPasswordCheck'
              }
              else {
          ...
          $.ajax({
          	type:'post',
          	url:'${pageContext.request.contextPath}/ForgetPasswordCheck.action',
          	data:{username:account},
          	dataType:'json',
          	success:function(data){
          		if(data.result=='success'){
          			var content='<div class="mui-content">'+
							        '<div id="slider" class="mui-slider">'+
							            '<dl style="margin: 40px;line-height: 30px; text-align: center;">'+
							                '<dt></dt>'+
							                '<dd>很抱歉，系统繁忙或出错！</dd>'+
							            '</dl>'+
							            '<div class=" bank_ann"><a href="${pageContext.request.contextPath}/home.action">返回首页</a></div>'+
							        '</div>'+
								'</div>';
