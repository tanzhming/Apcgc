...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>  <!-- EL 表达式函数库 -->
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
...
<div class="mui-content">
    <div class="news-lb">
        ...
        <c:forEach var="news" items="${newsList }" varStatus="vs">
                <li> 
                    <a href="${pageContext.request.contextPath }/NewsCommDetail.action?newsID=${news.id}&newsTitle=${news.newstitle}">
                    <c:if test="${fn:length(news.newstitle)>14 }">
                    	${fn:substring(news.newstitle,0,14) }...
                    </c:if>
                    <c:if test="${fn:length(news.newstitle)<=14 }">
                    	${news.newstitle }
                    </c:if>
