...
<header class="mui-bar mui-bar-nav">
    ...
    <h1 class="mui-title">行业资讯</h1>
</header>
<div class="mui-content">
    <div class="news-lb">

        <ul>
        <c:forEach var="innews" items="${allinnews }" varStatus="vs">
                <li>
                    <a href="${pageContext.request.contextPath }/NewsDetail.action?newsid=${innews.id}&title=${innews.innewstitle}">
						<c:if test="${fn:length(innews.innewstitle)>15 }">
							${fn:substring(innews.innewstitle,0,15) }...
						</c:if>
						<c:if test="${fn:length(innews.innewstitle)<15 }">
							${innews.innewstitle }
						</c:if>
                        <span class="mui-h5">
                        	<fmt:formatDate value="${innews.innewsdate }" pattern="yyyy/MM/dd"/>
                        </span>
